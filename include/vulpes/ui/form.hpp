...
enum class FormFieldKind { text, number, checkbox, lookup, date, time, date_time, read_only };
enum class FormMode { insert, edit };
enum class FormResult { unchanged, redraw, saved, cancelled, lookup_requested };

struct LookupOpenRequest {
    std::string field;
    model::LookupQuery query;
...
struct FormField {
    ...
    model::LookupQuery lookup_query;
    bool allow_drill_down{true};
    std::string error;
};
...
class RecordForm {
  public:
    ...
    [[nodiscard]] auto lookup_request() const -> std::optional<LookupOpenRequest>;
    void select_lookup(std::string_view field, model::LookupOption option);

...
auto Statement::bind(int index, const Value& value) -> Statement& {
    const int result = std::visit([&](const auto& item) -> int {
        using T = std::decay_t<decltype(item)>;
        if constexpr (std::is_same_v<T, std::monostate>) return sqlite3_bind_null(statement_, index);
        else if constexpr (std::is_same_v<T, std::int64_t>) return sqlite3_bind_int64(statement_, index, item);
        else if constexpr (std::is_same_v<T, double>) return sqlite3_bind_double(statement_, index, item);
        else if constexpr (std::is_same_v<T, std::string>) return sqlite3_bind_text64(statement_, index, item.data(), item.size(), SQLITE_TRANSIENT, SQLITE_UTF8);
        else return sqlite3_bind_blob64(statement_, index, item.data(), item.size(), SQLITE_TRANSIENT);
    }, value.storage());
...
auto Statement::bind(std::string_view name, const Value& value) -> Statement& {
    ...
    if (index == 0) throw Error{ErrorCategory::database, "unknown SQL parameter: " + owned_name};
...
auto Statement::column_count() const -> int { return sqlite3_column_count(statement_); }
...
auto Statement::column(int index) const -> Value {
    ...
    case SQLITE_INTEGER: return Value{sqlite3_column_int64(statement_, index)};
    case SQLITE_FLOAT: return Value{sqlite3_column_double(statement_, index)};
    case SQLITE_TEXT: {
    ...
    default: throw Error{ErrorCategory::database, "unsupported SQLite value type"};
    }
}

...
[[nodiscard]] auto is_text_field(const db::FieldSchema& field) -> bool {
    ...
    std::ranges::transform(type, type.begin(), [](unsigned char value) { return static_cast<char>(std::toupper(value)); });
    return type.find("CHAR") != std::string::npos || type.find("CLOB") != std::string::npos || type.find("TEXT") != std::string::npos;
}

[[nodiscard]] auto operator_sql(FilterOperator comparison) -> std::string_view {
    switch (comparison) {
    case FilterOperator::equal: return " = ";
    case FilterOperator::not_equal: return " <> ";
    case FilterOperator::less: return " < ";
    case FilterOperator::less_equal: return " <= ";
    case FilterOperator::greater: return " > ";
    case FilterOperator::greater_equal: return " >= ";
    }
    return {};
}
...
Dataset::Dataset(db::Database& database, db::TableSchema schema, std::size_t page_size)
    ...
    if (schema_.name.empty()) throw Error{ErrorCategory::database, "dataset requires a table or view name"};
    if (page_size_ == 0) throw Error{ErrorCategory::database, "dataset page size must be greater than zero"};
    refresh();
}

auto Dataset::total_count() -> std::size_t {
    std::vector<db::Value> values;
    auto query = database_->prepare("SELECT count(*) FROM " + db::detail::quote_identifier(schema_.name) + query_where_clause(values));
    for (std::size_t index = 0; index < values.size(); ++index) query.bind(static_cast<int>(index + 1), values[index]);
...
auto Dataset::current_identity() const -> std::optional<RowIdentity> {
    ...
    if (!row || fields.empty()) return std::nullopt;

    RowIdentity identity;
    identity.fields = fields;
    for (const auto& field : identity.fields) identity.values.push_back(row->at(field));
    return identity;
}

auto Dataset::is_editable() const noexcept -> bool { return !schema_.is_view && !schema_.primary_key_fields().empty(); }
...
auto Dataset::where(Filter filter) -> Dataset& {
    ...
    if (filter.value.is_null() && filter.comparison != FilterOperator::equal && filter.comparison != FilterOperator::not_equal) {
...
void Dataset::refresh() {
    std::vector<db::Value> values;
    const auto sql = "SELECT * FROM " + db::detail::quote_identifier(schema_.name) + query_where_clause(values) + order_clause() + " LIMIT ? OFFSET ?";
    auto query = database_->prepare(sql);
    int index = 1;
    for (const auto& value : values) query.bind(index++, value);
    ...
    rows_.clear();
    while (query.step()) rows_.push_back(query.row());
    current_index_ = 0;
}
...
auto Dataset::next() -> bool {
    ...
    if (page_offset_ + rows_.size() >= total_count()) return false;
...
auto Dataset::previous() -> bool {
    ...
    if (page_offset_ == 0) return false;
...
void Dataset::validate_field(std::string_view field) const {
    ...
    if (found == schema_.fields.end()) throw Error{ErrorCategory::validation, "unknown dataset field: " + std::string{field}};
}
...
auto Dataset::query_where_clause(std::vector<db::Value>& values) const -> std::string {
    ...
    if (search_ && !search_->empty() && has_text_fields()) {
        ...
        for (const auto& field : schema_.fields) {
            if (!field.hidden && is_text_field(field)) text_predicates.push_back(db::detail::quote_identifier(field.name) + " LIKE ? ESCAPE '\\'");
        }
        predicates.push_back("(" + [&] {
            std::string combined;
            for (std::size_t index = 0; index < text_predicates.size(); ++index) {
                if (index != 0) combined += " OR ";
...
auto Dataset::order_clause() const -> std::string {
    if (order_) {
        return " ORDER BY " + db::detail::quote_identifier(order_->first) + (order_->second == SortDirection::ascending ? " ASC" : " DESC");
    }
    const auto primary_key = schema_.primary_key_fields();
    if (primary_key.empty()) return {};

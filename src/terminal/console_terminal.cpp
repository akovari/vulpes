...
ConsoleTerminal::ConsoleTerminal() : implementation_{std::make_unique<Implementation>()} {
    ...
    const DWORD input_mode = (implementation_->input_mode | ENABLE_EXTENDED_FLAGS | ENABLE_WINDOW_INPUT) & ~ENABLE_QUICK_EDIT_MODE;
    ...
    raw.c_cc[VMIN] = 1;
    raw.c_cc[VTIME] = 0;
    if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) != 0) throw Error{ErrorCategory::terminal, "unable to configure terminal"};
#endif
...
auto ConsoleTerminal::size() const -> Size {
    ...
    if (!GetConsoleScreenBufferInfo(implementation_->output, &info)) throw Error{ErrorCategory::terminal, "unable to read console size"};
...
#else
    char byte{};
    if (read(STDIN_FILENO, &byte, 1) != 1) throw Error{ErrorCategory::terminal, "unable to read terminal input"};
    if (byte == '\x1B') {
        pollfd descriptor{.fd = STDIN_FILENO, .events = POLLIN, .revents = 0};
        if (poll(&descriptor, 1, 30) <= 0) return KeyEvent{.key = Key::escape};
        char prefix{};
        if (read(STDIN_FILENO, &prefix, 1) != 1 || prefix != '[') return KeyEvent{.key = Key::escape};
        char sequence{};
        if (read(STDIN_FILENO, &sequence, 1) != 1) return KeyEvent{.key = Key::escape};
        switch (sequence) {
        case 'A': return KeyEvent{.key = Key::up};
        case 'B': return KeyEvent{.key = Key::down};
        case 'C': return KeyEvent{.key = Key::right};
        case 'D': return KeyEvent{.key = Key::left};
        case 'H': return KeyEvent{.key = Key::home};
        case 'F': return KeyEvent{.key = Key::end};
        case '5': { char ignored{}; read(STDIN_FILENO, &ignored, 1); return KeyEvent{.key = Key::page_up}; }
        case '6': { char ignored{}; read(STDIN_FILENO, &ignored, 1); return KeyEvent{.key = Key::page_down}; }
        default: return KeyEvent{.key = Key::unknown};
        }
    }
    if (byte == '\r' || byte == '\n') return KeyEvent{.key = Key::enter};
    if (byte == '\t') return KeyEvent{.key = Key::tab};
    if (byte == 127) return KeyEvent{.key = Key::backspace};
    return KeyEvent{.key = Key::character, .character = static_cast<unsigned char>(byte), .ctrl = static_cast<unsigned char>(byte) < 32};
#endif
}

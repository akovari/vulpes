...
auto TestTerminal::read_event() -> InputEvent {
    if (events_.empty()) throw Error{ErrorCategory::terminal, "test terminal has no queued input event"};
...
void TestTerminal::enqueue(InputEvent event) { events_.push_back(std::move(event)); }
